...
const Productgalleryapi: React.FC = () => {
  ...
  }, [wishlist]); // Sync wishlist with localStorage

  const toggleWishlist = (product: Product) => {
    setWishlist((prevWishlist) => {
      const isAlreadyInWishlist = prevWishlist.some((item) => item._id === product._id);
      let updatedWishlist = isAlreadyInWishlist
    ...
    <main className="md:pt-10 mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-col-reverse md:flex-row">
        ...
        <div className="w-full md:w-1/5">
          ...
          <ul className="space-y-2 text-sm pr-5">
            {products.map((product) => (
              <li key={product._id} className="text-gray-600 hover:text-black cursor-pointer">
                {product.productName}
              </li>
            ))}
          </ul>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              ...
              >
                ...
                <p className="text-gray-400 sm:text-[8px] md:text-sm">{product.colors}</p>
                <p className="text-black sm:text-[8px] md:text-sm">MRP : ₹ {product.price}</p>
